...
func Chat(w http.ResponseWriter, r *http.Request) {
	...
	// 将 message 编码为 JSON 字节数组
	messageBytes, err := json.Marshal(message)
	if err != nil {
		log.Println("JSON 编码出错:", err)
		return
	}

	// 发送消息
	sendMsg(userId, messageBytes)
}
...
func dispatch(data []byte) {
	...
	fmt.Println("解析数据:", "msg.FormId", msg.FormId, "targetId:", msg.TargetId, "type:", msg.Type, "sendername", msg.SenderName)
	//fmt.Println("消息类型：", msg.Type, msg.Content)
	//判断消息类型

	// if msg.Type == 1 {
	// 	fmt.Println("私聊")
	// 	sendMsgAndSave(msg.TargetId, msg.Type, data)
	// } else if msg.Type == 2 {
	// 	fmt.Println("广播消息")
	// } else { //不等于1或者2就Type就算群聊信息，传递的数据就是群聊id
	// 	sendGroupMsg(uint(msg.FormId), uint(msg.TargetId), data, msg.Type)
	// }
	sendID := strconv.Itoa(int(msg.FormId))     //发送者
	targetId := strconv.Itoa(int(msg.TargetId)) //接收者
	var key string
	switch msg.Type {
	case 1: //私聊
		sendMsgAndSave(msg.TargetId, msg.Type, data)

		if msg.FormId > msg.TargetId {
			key = "msg_" + sendID + "_" + targetId
		} else {
			key = "msg_" + targetId + "_" + sendID
		}
		savemasge(key, data)
	case 2: //群发
		sendGroupMsg(uint(msg.FormId), uint(msg.TargetId), data, msg.Type)
...
// sendGroupMsg 群发
func sendGroupMsg(formId, target uint, data []byte, Type int) (int, error) {
	...
	for _, userId := range *userIDs {
		if formId != userId {
			fmt.Println("群发Type：", Type)
			sendMsgAndSave(int64(userId), Type, data)
		}
	}
	return 0, nil
}
...
func sendMsgAndSave(userId int64, Type int, msg []byte) {
	...
	// 	}
	// case 2:
	// 	userIdStr := strconv.Itoa(int(jsonMsg.TargetId))
	// 	if userId > jsonMsg.TargetId {
	// 		key = "gmsg_" + userIdStr + "_" + targetIdStr
	// 	} else {
	// 		key = "gmsg_" + targetIdStr + "_" + userIdStr
	// 	}
	// }
...
func savemasge(key string, msg []byte) {
	jsonMsg := Message{}
	json.Unmarshal(msg, &jsonMsg)
	ctx := context.Background()
...
// RedisMsg 获取缓存里面的消息
func RedisMsg(userIdA int64, userIdB int64, tpe int, start int64, end int64, isRev bool) []string {
	ctx := context.Background()
	userIdStr_groupId := strconv.Itoa(int(userIdA))
	...
	case 1:
		if userIdA > userIdB {
			key = "msg_" + userIdStr_groupId + "_" + targetIdStr

		} else {
			key = "msg_" + targetIdStr + "_" + userIdStr_groupId
		}
	case 2:
		key = "gmsg_" + userIdStr_groupId
	}
	if isRev {
