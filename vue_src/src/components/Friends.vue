...
import { SendOne, Plus } from '@icon-park/vue-next'
import { feiendsList, groupsList, readmsg, addFriend } from '@/api/manager'
import { getUserInfo } from '@/composables/auth'
const userinfo = getUserInfo()
const friends = ref([]) // 好友列表
const groups = ref([]) // 群列表
const selectedFriend = ref(false) // 当前选中的好友
...
function handleIncomingMessage(event: MessageEvent) {
  try {
    ...
    if (
      ...
      incomingMessage.userId === friendId.value
    ) {
      messages.value.push(incomingMessage)
    } else if (incomingMessage.Type === 1) {
...
const addfriend = () => {
  addFriend(addFriendid.value).then(res => {
    ElNotification({
      title: 'Success 成功',
      message: `成功添加好友：${addFriendid.value}`,
      type: 'success',
      duration: 1500,
    })
  })
...
const buttonStyle = computed(() => {
  return {
    backgroundColor: addFriendid.value ? '#6366f1' : '#aaa', // 根据输入框是否有内容改变颜色
    cursor: addFriendid.value ? 'pointer' : 'not-allowed', // 根据是否有内容改变鼠标指针样式
  }
})
</script>
        ...
        <div class="friend-list-container">
          <template v-if="filteredFriends.length > 0">
            <div
              ...
              :style="{
                backgroundColor:
                  friendId === friend.UserID ? '#6366f1' : '#323645',
              }"
            >
          ...
          <template v-else>
            <p class="no-friend-found">没有好友</p>
          </template>
        </div>
      </div>
    </el-col>
    ...
    <el-col :span="20">
      <div class="chat-container">
        <div v-if="selectedFriend">
          <h2>{{ matchfriends(friendId)?.Name }}</h2>
        ...
        <div v-else class="chat-box-placeholder">请选择好友开始聊天</div>
      </div>
    </el-col>
  </el-row>
  ...
  <div v-if="dialogopen" class="dialog-overlay">
    <div class="custom-dialog" @click.stop>
      <div class="dialog-header">
        <div class="close-buttons">
          <div
            ...
            @click="
              (dialogopen = false), (showCloseIcon = false), clearaddfriend()
            "
          >
      ...
      <div class="dialog-footer">
        <button
          @click="(dialogopen = false), clearaddfriend()"
          class="cancel-button"
        >
          取消
        </button>
        <button
          @click="addfriend(), (dialogopen = false)"
          :style="buttonStyle"
